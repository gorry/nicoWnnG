...
public class RomkanFullKatakana implements LetterConverter {
	...
	private static final HashMap<String, String> mRomkanTable2 = new HashMap<String, String>() {{
		put("n", "\u30f3");
	}};
	...
	public boolean convert2(final ComposingText text) {
		return convert(text, mRomkanTable2);
	}
	 ...
	 */
	public static boolean convert(final ComposingText text, final HashMap<String, String> table) {
		...
		while (start < 3) {
			...
			if (match != null) {
				...
				final Matcher m = p.matcher(text.toString(ComposingText.LAYER1));
				if (m.matches()) {
