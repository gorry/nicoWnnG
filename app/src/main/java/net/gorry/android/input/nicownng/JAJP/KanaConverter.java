...
public class KanaConverter {
	...
	private final List<WnnWord> mAddCandidateList;
	/** Work area for generating string */
	private final StringBuffer mStringBuff;
	 ...
	 */
	public void setDictionary(final WnnDictionary dict) {
	...
	public String Hiragana2FullKatakanaAndCapitalizeAlphabet(final String inputHiragana) {
		final StringBuffer sb = new StringBuffer();
		if (createCandidateString(inputHiragana, mFullKatakanaCapitalizeAlphabetMap, sb)) {
			return sb.toString();
		}
		return inputHiragana;
	}

	public String Hiragana2HalfKatakana(final String inputHiragana) {
		final StringBuffer sb = new StringBuffer();
		if (createCandidateString(inputHiragana, mHalfKatakanaMap, sb)) {
			return sb.toString();
		}
		return inputHiragana;
	}
	 ...
	 */
	public List<WnnWord> createPseudoCandidateList(final String inputHiragana, final String inputRomaji, final int keyBoardMode, final int addSpecialCandidateOnKana12KeyToggleMode) {
		final List<WnnWord> list = mAddCandidateList;
		...
		if ((keyBoardMode == NicoWnnGEngineJAJP.KEYBOARD_QWERTY) || (addSpecialCandidateOnKana12KeyToggleMode == 0)) {
			...
			if (addSpecialCandidateOnKana12KeyToggleMode == 2) {
				/* Create pseudo candidates for half width numeric */
				if (createCandidateString(inputHiragana, mHalfNumericMap2, mStringBuff)) {
					final String convHanSuuji = mStringBuff.toString();
					final String convNumComma = convertNumber(convHanSuuji);
				...
				if (createCandidateString(inputHiragana, mFullNumericMap2, mStringBuff)) {
				...
				if (createCandidateString(inputHiragana, mHalfAlphabetMap, mStringBuff)) {
					final String convHanEiji = mStringBuff.toString();
					final String convHanEijiLower = convHanEiji.toLowerCase();
				...
				if (createCandidateString(inputHiragana, mFullAlphabetMap, mStringBuff)) {
					final String convZenEiji = mStringBuff.toString();
					final String convZenEijiLower = convZenEiji.toLowerCase(Locale.JAPAN);
	 ...
	 * @param inputRomaji       The input string (Romaji)
	 */
	private void createPseudoCandidateListForQwerty(final String inputHiragana, final String inputRomaji) {
		final List<WnnWord> list = mAddCandidateList;

		/* Create pseudo candidates for half width alphabet */
		final String convHanEijiLower = inputRomaji.toLowerCase();
	 ...
	 */
	private boolean createCandidateString(final String input, final HashMap<String,String> map, final StringBuffer outBuf) {
		if (outBuf.length() > 0) {
			outBuf.delete(0, outBuf.length());
		}
		for (int index = 0; index < input.length(); index++) {
			final String convChar = map.get(input.substring(index, index + 1));
			if (convChar == null) {
				return false;
			}
	 ...
	 */
	private String convertCaps(final String moji) {
		String tmp = "";
	 ...
	 * @return          {@code true} if success
	 */
	private String convertNumber(final String numComma) {
		try {
	...
	/** Conversion rule for half-width numeric 2 */
	private static final HashMap<String,String> mHalfNumericMap2 = new HashMap<String,String>() {{
		put( "\u3042", "1");
		put( "\u3044", "1");
		put( "\u3046", "1");
		put( "\u3048", "1");
	...
	/** Conversion rule for full-width numeric 2 */
	private static final HashMap<String,String> mFullNumericMap2 = new HashMap<String,String>() {{
		put( "\u3042", "\uff11");
		put( "\u3044", "\uff11");
		put( "\u3046", "\uff11");
		put( "\u3048", "\uff11");
	...
	/** Conversion rule for full-width Katakana AND Capitalize Alphabet */
	private static final HashMap<String,String> mFullKatakanaCapitalizeAlphabetMap = new HashMap<String,String>() {{
		put( "\u3042", "\u30a2");
		put( "\u3044", "\u30a4");
		put( "\u3046", "\u30a6");
		put( "\u3048", "\u30a8");
